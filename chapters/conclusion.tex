...
Les quatre itérations ont livré les réalisations suivantes~:

\begin{itemize}
  \item \textbf{Itération~1 --- Backend}~: API REST complète (30+ endpoints), 7 modèles
    MongoDB, architecture de sécurité à 4 couches (JWT différencié, RBAC, bcrypt,
    SHA-256), système d'activation de casques (identifiant Meta User ID + code à 6 chiffres), login employé
    (accessCode XX-001 + PIN). Déployé sur Render.

  \item \textbf{Itération~2 --- Dashboards}~: dashboard Admin (7 pages, React + TypeScript)
  ...
  \item \textbf{Itération~3 --- Unity Auth}~: scène tutoriel (\texttt{trainingscene}, 3 leçons) et
    scène d'authentification (\texttt{Companys})~: activation casque, login employé, sélection formations, paramètres.
    Architecture Unity avec TynassApiClient, AppSession, PanelManager.

  \item \textbf{Itération~4 --- Simulation Indigo}~: simulation VR d'intégration des
    nouveaux employés (2 phases~: exploration des locaux et quiz), soumission
...
\begin{itemize}
  ...
  \item \textbf{Gestion de projet}~: approche itérative et incrémentale, backlog MoSCoW, priorisation.
...
\begin{itemize}
  ...
  \item \textbf{OAuth Meta Quest}~: authentification via le compte Meta de l'employé,
   ...
   \item \textbf{Nouveaux modules de formation}~: extension du catalogue, à commencer par
    d'autres modules pour Indigo Company (règlement intérieur, pointage, sécurité), puis pour
    d'autres clients B2B (procédures logistiques, formation à la vente).
