...
Ce sprint couvre le développement de l'application Unity~6 pour Meta Quest~3,
incluant les deux premières scènes~: \textbf{trailler} (tutoriel des contrôleurs)
et \textbf{Companys} (authentification et sélection des formations). À l'issue de ce sprint,
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{llXl}
    ...
    \midrule
    US-14 & \must{}   & Scène trailler --- tutoriel 3 leçons & \checkmark \\
    US-15 & \must{}   & DeviceActivationManager (Meta User ID) & \checkmark \\
    US-16 & \must{}   & EmployeeSignInPanel (code + PIN numpad) & \checkmark \\
    US-17 & \must{}   & ModulesPanel (grille formations) & \checkmark \\
    US-18 & \should{} & SettingsPanel (audio, langue, contrôleurs) & \checkmark \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{archbox}
  ...
  sans instanciation, sans DontDestroyOnLoad, sans risque de destruction entre scènes.
  Pour des données d'état global (token, employé connecté, formation active),
  la classe statique est architecturalement plus appropriée.
\end{archbox}
...
\begin{itemize}
  \item \textbf{TynassApiClient.cs}~: singleton HTTP centralisé. Gère le JWT
    (\texttt{SetToken/ClearToken}), \texttt{TryRestoreSession()} au démarrage,
    et toutes les méthodes typées de l'API. Utilise les classes \texttt{[Serializable]}
    pour la sérialisation JSON via \texttt{JsonUtility}.
  \item \textbf{AppSession.cs}~: classe statique globale. Stocke les données de
    ...
    QuizCurrentIndex, QuizElapsedSecs). \texttt{AppSession.Clear()} efface les données
    personnelles de l'employé mais conserve le \texttt{DeviceToken} (appartient au casque).
  ...
  \item \textbf{DeviceActivationManager.cs}~: récupère le Meta User ID via
    \texttt{Oculus.Platform.\allowbreak Users.\allowbreak GetLoggedInUser()}, orchestre le flow d'activation.
  \item \textbf{EmployeeSignInPanel.cs}~: gère les 2 étapes de login
    (Step 1: accessCode XX-001, Step 2: PIN numpad 4 chiffres).
  ...
  \item \textbf{SettingsPanel.cs}~: audio (master/sfx/narration/spatial), langue
    (FR/EN/AR), contrôleurs (main, dwell time, haptic). Persistance via PlayerPrefs.
  \item \textbf{TrainingSceneManager.cs}~: orchestre la scène trailler
    (leçons, narration audio, NarrationManager).
\end{itemize}
...
\begin{ucbox}{Tutoriel d'introduction (scène trailler)}
  \begin{tabular}{lp{9cm}}
    ...
    \textbf{Scénario A}     &
      1. L'utilisateur choisit \og Commencer les leçons \fg (choix A). \newline
    ...
    \textbf{Scénario B}     &
      1. L'utilisateur choisit \og Passer \fg (choix B). \newline
...
\begin{ucbox}{Sélectionner une formation (ModulesPanel)}
  \begin{tabular}{lp{9cm}}
    \textbf{Pré-conditions} & Employé connecté, device\_token valide. \\
    \textbf{Acteur}         & Employé. \\
    \textbf{Scénario}       &
...
\begin{enumerate}
  \item \textbf{Step 1 --- accessCode}~: l'employé saisit son code via un clavier
    ...
    \texttt{Regex.IsMatch(code, \texttt{\^}[A-Z]\{2\}-\textbackslash d\{3\}\$)}.
