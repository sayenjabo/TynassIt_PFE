...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{cp{3.5cm}Xcc}
    \toprule
    \textbf{ID} & \textbf{Titre} & \textbf{Tâches principales} & \textbf{MoSCoW} & \textbf{\footnotesize Charge estimée (j/h)} \\
    \midrule
...
complémentaires, illustrée par la figure~\ref{fig:securite}. Chaque couche filtre la
requête à un niveau différent, de l'authentification du jeton jusqu'à la sécurité du
casque physique.
...
La figure~\ref{fig:seq_login} détaille la séquence de connexion d'un employé sur le
casque~: la saisie du code d'accès et du PIN, leur validation côté serveur, puis le
...
\begin{verbatim}
backend/
|-- index.js               point d'entree + montage des routes
...
|   |-- company.auth.controller.js
|   |-- admin.companies.controller.js
|   |-- admin.trainings.controller.js
|   |-- device.controller.js
|   |-- employee.controller.js
|   |-- session.controller.js
|   +-- quiz.controller.js
|-- routes/                points d'entree REST par acteur
|   |-- admin.auth.routes.js
|   |-- company.auth.routes.js
|   |-- admin.companies.routes.js
|   |-- admin.trainings.routes.js
|   |-- admin.devices.routes.js
|   |-- device.routes.js
|   |-- device.sessions.routes.js
|   |-- employee.routes.js
|   |-- session.routes.js
|   |-- quiz.routes.js
|   +-- company.trainings.routes.js
|-- middleware/
|   +-- auth.middleware.js     protect, adminOnly, superAdminOnly,
|                              companyOnly, deviceOnly
+-- .env                   variables d'environnement
\end{verbatim}
\end{minipage}
\caption{Arborescence du projet backend (architecture MVC)}
\label{fig:backend_arbo}
\end{figure}
...
\begin{itemize}
  ...
  \item \texttt{controllers/}~: logique métier séparée par domaine (admin.auth,
    company.auth, admin.companies, admin.trainings, session, employee, device, quiz).
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l>{\footnotesize}p{7.5cm}Xc}
    \toprule
...
La figure~\ref{fig:postman_check} montre un appel à l'endpoint de vérification
du casque testé sous Postman, avec la réponse retournée par le backend.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{../images/postman_deviceCheck.png}
  \caption{Test de l'endpoint d'activation sous Postman}
  \label{fig:postman_check}
\end{figure}

La figure~\ref{fig:compass} présente les collections de la base de données
telles qu'elles apparaissent dans MongoDB Compass.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{../images/compass_collections.png}
  \caption{Collections de la base de données dans MongoDB Compass}
  \label{fig:compass}
\end{figure}
