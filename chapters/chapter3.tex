...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{llXl}
    ...
    \midrule
    US-01 & \must{} & Modèle Admin + routes auth + JWT admin & \checkmark \\
    US-02 & \must{} & Modèle Company + CRUD + routes admin & \checkmark \\
    US-03 & \must{} & Modèle Training + CRUD + sceneName & \checkmark \\
    US-04 & \must{} & Modèle Device + activation (Meta User ID) + SHA-256 & \checkmark \\
    US-05 & \should{} & Agrégations MongoDB pour statistiques & \checkmark \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\subsubsection{Choix de MongoDB}
...
\begin{notebox}
  ...
  Employee référencés dans Session) sont des \textbf{références ObjectId}.
\end{notebox}
...
\tynassit{} implémente une architecture de sécurité organisée en \textbf{4 couches
...
Le payload JWT encode le type d'acteur~: \texttt{\{id, type: 'admin'|'company'|'device',
role?\}}. Cette différenciation garantit qu'un token de type \texttt{device} ne peut jamais
accéder aux routes réservées aux admins, même en cas d'interception.
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Admin (superadmin / staff) & \texttt{admin} & 7 jours \\
    Company (dashboard)        & \texttt{company} & 7 jours \\
    Device (casque Meta Quest~3)& \texttt{device} & 1 an \\
    \bottomrule
  \end{tabular}
\end{table}

Le token device a une durée d'un an car le casque est un device fixe appartenant à
l'entreprise (pas à l'employé individuel). Supprimer le token à chaque changement d'employé
forcerait une réactivation à chaque session, ce qui est opérationnellement absurde.
...
\begin{itemize}
  ...
  \item \textbf{SHA-256 pour les tokens device}~: le token brut (plain) est exposé
    \textbf{une seule fois}, pendant 60~secondes. Seul son hash est stocké en base.
    En cas de compromission de la BDD, les tokens hachés sont inutilisables.
  \item \textbf{SHA-256 pour le reset password}~: même logique, avec expiration de 30~min.
\end{itemize}
...
\begin{ucbox}{Activer un casque Meta Quest~3}
  \begin{tabular}{lp{9cm}}
    ...
    \textbf{Scénario}       &
      ...
      6. Token stocké dans \texttt{PlayerPrefs} (permanent). \\
...
\begin{ucbox}{Login employé via casque VR}
  \begin{tabular}{lp{9cm}}
    ...
    \textbf{Scénario}       &
      ...
      4. Unity envoie \texttt{POST /api/company/employees/login} avec device\_token. \newline
      5. Backend vérifie le PIN (bcrypt.compare) et retourne employee + auth\_token. \newline
      6. AppSession stocke les données employé. \\
    \textbf{Exceptions}     & Code invalide → message d'erreur. PIN incorrect → rejet. \\
    \textbf{Post-conditions}& Employé connecté, AppSession initialisée, ModulesPanel chargé. \\
  \end{tabular}
\end{ucbox}
...
\begin{ucbox}{Soumettre une session de formation}
  \begin{tabular}{lp{9cm}}
    ...
    \textbf{Scénario}       &
      ...
      4. Backend calcule durationSeconds (côté serveur, horloge fiable). \newline
...
Le projet backend est organisé selon le pattern \textbf{MVC}~:

\begin{itemize}
  \item \texttt{models/}~: 7 schémas Mongoose (Company, Admin, Training, Session,
    Employee, Device, Counter).
  \item \texttt{controllers/}~: logique métier séparée par domaine
    (admin.auth, company.auth, admin.companies, admin.trainings,
    session, employee, device).
  \item \texttt{routes/}~: points d'entrée REST groupés par acteur.
  \item \texttt{middleware/}~: auth.middleware.js avec les 5 guards.
  \item \texttt{utils/}~: sendEmail.js (Nodemailer).
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{llXl}
    ...
    POST & \texttt{/api/sessions/submit}            & Soumettre une session     & Auth \\
    GET  & \texttt{/api/sessions/stats/trainings}   & Stats par formation       & Admin \\
    GET  & \texttt{/api/sessions/stats/companies}   & Stats par entreprise      & Admin \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Le système d'activation est la fonctionnalité la plus originale du backend. Il repose sur
l'identifiant hardware \textbf{Meta User ID}, fourni par le SDK Oculus Platform et impossible
à falsifier. Le flow complet est le suivant~:
...
\begin{enumerate}
  ...
  \item Le backend vérifie le code, génère un JWT device (1~an), stocke le hash SHA-256
    en BDD et expose le token brut pendant 60~secondes maximum.
  \item Unity appelle \texttt{POST /check}, récupère le token brut et le stocke dans
    \texttt{PlayerPrefs}. Le token brut est immédiatement effacé côté serveur.
\end{enumerate}

\subsection{Fonctionnalité clé --- Compteur atomique (accessCode)}

L'accessCode des employés suit le format \textbf{XX-001} (2 lettres initiales de
l'entreprise + numéro auto-incrémenté). Pour éviter les race conditions lors de créations
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXX}
    ...
    \midrule
    Race condition & Deux employés créés simultanément pouvaient
                     recevoir le même accessCode (countDocuments
                     non atomique). &
                     Migration vers Counter + \texttt{\$inc} MongoDB~:
                     opération atomique garantissant l'unicité. \\
    \midrule
    Sécurité token device & Stocker le device token en clair en BDD
                            représentait un risque en cas de compromission. &
                            SHA-256 permanent pour le stockage, token brut
                            exposé 60~s uniquement puis effacé. \\
    \midrule
    UnityWebRequest 4xx & Unity traitait les réponses HTTP 4xx comme des
                          erreurs réseau sans lire le body JSON. &
