...
puis le backlog produit avec priorisation \textbf{MoSCoW} et la planification des itérations.

\section{Analyse des besoins}

Cette section identifie les acteurs du système puis détaille leurs besoins fonctionnels
et non-fonctionnels.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.6cm}X}
    ...
    & Gérer les comptes Staff Admin (créer, modifier, activer, désactiver). \\
    & Accéder à toutes les fonctionnalités du Staff Admin. \\
    & Supprimer des entreprises et des formations (action réservée au Superadmin).\\
    \midrule
    \multirow{6}{=}{\textbf{Staff Admin}} &
      S'authentifier sur la plateforme web. \\
    & Gérer les entreprises clientes (créer, modifier, activer, désactiver). \\
    & Gérer le catalogue de formations VR (créer, modifier ; champ \texttt{sceneName}). \\
    & Activer un casque Meta Quest~3 en saisissant le code à 6 chiffres affiché sur le casque. \\
    & Assigner des formations à une entreprise. \\
    & Consulter les statistiques et sessions de toutes les entreprises. \\
    \midrule
    \multirow{6}{=}{\textbf{Company}} &
      S'authentifier sur le dashboard entreprise. \\
    & Gérer les employés (créer, modifier, supprimer), générer leur code d'accès et définir un code PIN protégé par hachage en base. \\
    & Créer et gérer un quiz par formation (questions, choix, seuil de réussite). \\
    & Définir des objectifs (\textit{milestones}) de formation par employé. \\
    & Révoquer un casque Meta Quest~3. \\
    & Consulter les sessions et performances de ses employés. \\
    \midrule
    \multirow{5}{=}{\textbf{Employé}} &
      Se connecter au casque (code d'accès au format XX-001 + PIN à 4 chiffres). \\
    & Accéder aux formations assignées à son entreprise. \\
    & Suivre le tutoriel d'initiation à l'utilisation des contrôleurs. \\
    & Lancer la simulation d'intégration Indigo. \\
...
parmi lesquelles nous retenons celles les plus pertinentes pour ce projet.

\placeholder{Modèle ISO/IEC 25010 (figure)}

Les caractéristiques retenues pour \tynassit{} sont détaillées ci-dessous.

\subsubsection{Efficience de la performance --- Comportement temporel}
...
\subsubsection{Fiabilité --- Disponibilité}
...
\subsubsection{Maintenabilité --- Modifiabilité}
...
\subsubsection{Utilisabilité --- Apprentissage}

\begin{itemize}
  \item \textbf{Tutoriel VR}~: un tutoriel de trois leçons couvrant les interactions
...
La modélisation du système s'appuie sur le langage UML et se décline en trois vues
complémentaires~: fonctionnelle, structurelle et dynamique.

\subsection{Vue fonctionnelle --- Diagramme de cas d'utilisation}

Le diagramme de cas d'utilisation de la figure~\ref{fig:cas_utilisation} présente les
quatre acteurs du système et les fonctionnalités accessibles à chacun. Le Superadmin
hérite de toutes les capacités du Staff Admin, auxquelles s'ajoutent la gestion des
comptes administrateurs et la suppression d'entreprises et de formations.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{../images/casUtilisation.png}
...
La figure~\ref{fig:classes} présente le diagramme de classes des huit modèles de données
de la plateforme. Il distingue les entités principales, référencées entre elles par des
identifiants (\texttt{ObjectId}), des sous-documents imbriqués (\textit{embedded}) qui
n'ont pas d'existence indépendante, comme les objectifs d'un employé ou les critères
d'évaluation d'une session.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{../images/diagramClasses.png}
  \caption{Diagramme de classes des huit modèles de données}
  \label{fig:classes}
\end{figure}

\section{Architecture du système}

L'architecture de \tynassit{} est présentée sous deux angles~: une vue logique en
couches, puis une vue physique décrivant le déploiement des composants.
...
\begin{itemize}
  ...
  \item \textbf{Couche données}~: MongoDB Atlas avec 8 collections.
\end{itemize}

La figure~\ref{fig:archi_couches} illustre l'architecture logique de la plateforme,
organisée en trois couches.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../images/archiCouches.png}
  \caption{Architecture logique en trois couches}
  \label{fig:archi_couches}
\end{figure}

Cette séparation en couches garantit que la couche présentation (dashboards et application
VR) ne communique jamais directement avec la base de données, mais uniquement à travers
l'API métier, qui centralise la logique et les contrôles de sécurité.

\subsection{Architecture physique --- Diagramme de déploiement}

La figure~\ref{fig:deploiement} présente le diagramme de déploiement, décrivant la
répartition physique des composants sur les différents services d'hébergement cloud.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{../images/deploiement.png}
  \caption{Diagramme de déploiement}
  \label{fig:deploiement}
\end{figure}

\noindent Le système s'appuie sur trois environnements cloud, l'application VR
étant déployée directement sur le casque~:
...
\section{Pilotage et planification du projet}

Cette section décrit la démarche de développement adoptée, le backlog produit priorisé
selon la méthode MoSCoW, et la répartition du travail sur les quatre itérations.

\subsection{Démarche itérative et incrémentale}

La figure~\ref{fig:processus_iteratif} illustre le cycle appliqué à chaque itération~:
une phase d'analyse et de conception, le développement de l'incrément, une phase de test
et de correction, puis la livraison, avant de passer à l'itération suivante.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{../images/processusIteratif.png}
  \caption{Processus de développement itératif et incrémental}
  \label{fig:processus_iteratif}
\end{figure}

\subsection{Backlog produit}

Le backlog produit regroupe \textbf{25 user stories} organisées en \textbf{5 épics},
...
\begin{longtable}{lp{2.8cm}p{8cm}l}
  ...
  US-24 & Gestion des quiz &
    En tant qu'entreprise, je veux créer et configurer un quiz par
    formation, afin d'évaluer la compréhension de mes employés.
    \newline\textbf{Critères d'acceptation}~:
    \begin{itemize}
      \item CRUD complet des questions, choix et bonne réponse.
      \item Définition d'un seuil de réussite (\textit{passing score}).
      \item La bonne réponse n'est jamais transmise au casque.
    \end{itemize}
    & \must{} \\[4pt]

  US-25 & Objectifs de formation (milestones) &
    En tant qu'entreprise, je veux définir des objectifs de formation
    par employé, afin de suivre leur progression individuelle.
    \newline\textbf{Critères d'acceptation}~:
    \begin{itemize}
      \item Création d'objectifs par employé (titre, type, module).
      \item Passage automatique de « en attente » à « atteint » après une
        session réussie.
      \item Objectifs visibles sur la fiche de l'employé.
    \end{itemize}
    & \should{} \\[4pt]
  ...
  US-19 & Exploration des locaux &
    ...
    \newline\textbf{Critères d'acceptation}~:
    \begin{itemize}
      \item Les sept départements et la zone du responsable sont accessibles (huit zones).
  ...
  US-20 & Quiz de fin de simulation &
    ...
    \newline\textbf{Critères d'acceptation}~:
    \begin{itemize}
      \item Le nombre de questions et leur contenu sont définis par l'entreprise depuis son tableau de bord.
      \item Le score est calculé côté serveur et exprimé en pourcentage.
      \item Le seuil de réussite (70\% par défaut) est défini par l'entreprise.
    \end{itemize}
    & \must{} \\[4pt]
...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    \toprule
    \textbf{Itération} & \textbf{Titre} & \textbf{Durée} & \textbf{US couvertes} \\
    \midrule
    Itération 1 & Backend Core \& Sécurité          & 3 semaines & US-01 à US-05 \\
    Itération 2 & Dashboards Admin \& Company        & 3 semaines & US-06 à US-13 \\
    Itération 3 & Application VR Unity (Auth)        & 4 semaines & US-14 à US-18 \\
    Itération 4 & Simulation VR d'intégration Indigo & 5 semaines & US-19 à US-25 \\
    \bottomrule
  \end{tabular}
  \caption{Planification des itérations}
  \label{tab:sprint_plan}
\end{table}

\begin{notebox}
  Le projet suit une approche \textbf{itérative et incrémentale}~: chaque itération
  livre un incrément fonctionnel exploitable, en s'appuyant sur les livrables des
  itérations précédentes (l'application VR ne peut être développée avant que le backend
  existe, par exemple). Les durées (3, 3, 4 et 5 semaines) sont fixées par
  \textit{timeboxing}. Le regroupement des user stories par itération suit leurs
  dépendances techniques, et non une correspondance stricte avec les épics du backlog.
  Ainsi, les fonctionnalités de quiz et d'objectifs (US-24 et US-25), bien que rattachées
  à l'épic du dashboard Company, ont été développées lors de la quatrième itération, en
  complément de la simulation Indigo qui les exploite.
\end{notebox}
...
architecture déployée et backlog MoSCoW de 25 user stories. Le chapitre suivant présente
la réalisation de la première itération~: le backend core et son architecture de sécurité.
